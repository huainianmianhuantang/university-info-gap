...
- **关注与信息渠道**：每所高校详情页整合 官网 / B 站 / 小红书 / 抖音 / 微博 入口（官网直达，其余为搜索入口，避免编造账号，后续可替换为官方账号直达链接）。
...
- **选校测评 `/quiz/`**：学生 / 家长 × 轻量 / 全面四套问卷 → 推荐专业方向，双视角各 3 所高校 + 选校建议。结果保存在浏览器 localStorage。
- **账号与登录**：手机号 + 验证码（开发模式展示演示码）/ 密码登录，QQ / 微信绑定入口（真实接入需开发者资质，当前为演示模式）；不登录也能浏览全部内容，登录后可进入个人中心（收藏 / 测评历史 / 咨询记录聚合）、投稿自动带身份、小助手个性化问候。数据当前存储于本机（localStorage），云端同步待接入。
- **站内小助手**：全站右下角 💬 悬浮窗，可快速了解网站用法、选校思路、转专业等，回答结合站内栏目并给出页面路径，支持咨询历史与快捷问题。
...
- 选校建议：`src/lib/ai.ts` 提供规则引擎；`/api/advice` 服务端接口可选接入大模型（见下）。

### 个性化建议接口（可选，默认 DeepSeek）
...
3. 测评结果页会先展示规则引擎建议，随后自动升级为接口生成的分点建议；接口异常时自动回退规则引擎。
